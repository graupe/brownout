...
static float gammar(int percent) { return (.9 + .1*.01*percent); }
static float gammag(int percent) { return (.4 + .1*.01*percent + .5*exp((.01*percent-1)/4.0)); }
static float gammab(int percent) { return (.0 + .1*.01*percent + .9*exp( .01*percent-1)); }
...
static float avg(int percent, int color)
{
	...
	float c, cn;
	switch(color) {
		case 0: c = wp[0].r, cn = wp[1].r; break;
		case 1: c = wp[0].g, cn = wp[1].g; break;
		default: c = wp[0].b, cn = wp[1].b; break;
	}
	return c * (1.0 - avgratio) + cn * avgratio;
}
static float gammar(int percent) { return avg(percent, 0); }
static float gammag(int percent) { return avg(percent, 1); }
static float gammab(int percent) { return avg(percent, 2); }
#endif
